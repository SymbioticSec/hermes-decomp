        ...
        if i + 1 < len {
            ...
            } = stmt
            {
                if is_esmodule_marker_obj(value) {
                    ...
                    if let Statement::Assign {
                        target: AssignTarget::Binding(Binding::Variable(name2)),
                        ..
                    } = &stmts[i + 1]
                    {
                        if name == name2 {
...
fn is_define_property_esmodule(stmt: &Statement) -> bool {
    ...
    if let Expression::Call { callee, arguments } = value {
        ...
        let is_define_prop = match callee.as_ref() {
            Expression::Member {
                property: crate::ir::PropertyKey::Ident(p),
                ..
            } => p == "defineProperty",
            _ => false,
        };
...
pub(super) fn inline_hoisted_aliases_and_trim(stmts: Vec<Statement>) -> Vec<Statement> {
    ...
    let term_idx = stmts
        .iter()
        .position(|s| matches!(s, Statement::Return(_) | Statement::Throw(_)));
    ...
    match stmt {
        ...
        Statement::If {
            ...
            for s in then_body.iter_mut() {
                apply_hoisted_aliases_to_stmt(s, aliases);
            }
...
fn apply_hoisted_aliases_to_target(
    ...
    match expr {
        ...
        Expression::Call { callee, arguments } => {
            ...
            for a in arguments.iter_mut() {
                apply_hoisted_aliases_to_expr(a, aliases);
            }
        }
        ...
        Expression::Array { elements } => {
            for item in elements.iter_mut().flatten() {
                apply_hoisted_aliases_to_expr(item, aliases);
            }
        }
        Expression::Object { properties } => {
            for prop in properties.iter_mut() {
                apply_hoisted_aliases_to_expr(&mut prop.value, aliases);
            }
        }
