...
pub(crate) fn finalize_module_specifiers(
    ...
    for (id, module) in registry.modules.iter_mut() {
        ...
        // The default export's own function name is the module, even when it
        // reads as an action: date-fns/toDate exports `function toDate`.
        if !locked.contains(id) && !module.name_from_default_export && names_an_action(name) {
            module.name = None;
        }
    }
    ...
    for (name, mut ids) in by_name {
        ...
        // A module carrying the name as its default export's function name
        // is a copy of one helper (`_slicedToArray` in eight packages): the
        // name is right for each, and the Metro id comment tells them apart.
        // The other sharers (an interop wrapper re-exporting one of them)
        // cannot claim it and go back to the honest id.
        if ids
            .iter()
            .any(|id| registry.modules[id].name_from_default_export)
        {
            clears.extend(ids.iter().copied().filter(|id| {
                !registry.modules[id].name_from_default_export && !locked.contains(id)
            }));
            continue;
        }
...
pub(crate) fn is_obviously_generic(name: &str) -> bool {
    ...
    if is_role_name_with_digits(name) {
        return true;
    }
    false
}
...
fn is_role_name_with_digits(name: &str) -> bool {
    const ROLE_BASES: &[&str] = &[
        "obj", "arr", "items", "item", "set", "list", "map", "fn", "num", "str", "val", "bool",
        "res", "el",
    ];
...
impl MetroRegistry {
    pub fn analyze(statements: &[crate::ir::Statement]) -> Self {
        ...
        MetroDetector::analyze_statements(
            statements,
            &mut registry,
...
pub(crate) fn names_an_action(name: &str) -> bool {
    const VERBS: &[&str] = &[
        "get", "set", "is", "has", "use", "add", "on", "create", "make", "fetch", "load", "save",
        "read", "write", "parse", "format", "handle", "remove", "delete", "clear", "reset",
        "update", "init", "build", "check", "ensure", "with", "to", "from", "can", "should",
        "will", "did",
    ];
    VERBS.iter().any(|verb| {
...
mod generic_name_tests {
    ...
    fn rejects_role_names_with_any_digit_count() {
        for n in [
            "obj", "obj2", "obj129", "items9", "set7", "arr12", "fn3", "num2", "str1", "val0",
            "map", "list5", "res4",
        ] {
    ...
    fn rejects_placeholder_module_specifiers() {
        ...
        use super::{
            finalize_module_specifiers, is_generic_module_specifier, is_usable_module_specifier,
        };

        for n in [
            "clear", "keys", "result", "store", "callback", "set", "size", "index", "obj2",
            "module_2",
        ] {
            assert!(
                is_generic_module_specifier(n),
                "{n} should be a generic specifier"
            );
            assert!(
                !is_usable_module_specifier(n) || n == "module_2",
                "{n} usable?"
            );
        }
        ...
        registry
            .modules
            .insert(6, mk(6, Some("LoggerPIIRestrictedObjects")));
        ...
        assert_eq!(
            registry.modules[&6].name.as_deref(),
            Some("LoggerPIIRestrictedObjects")
        );
        assert_eq!(
            registry.modules[&10].name.as_deref(),
            Some("SnowflakeUtils")
        );
    ...
    fn drops_stems_shared_by_many_modules() {
        ...
        for &id in &shared {
            assert_eq!(
                registry.modules[&id].name, None,
                "module {id} kept a shared stem"
            );
        }
        ...
        finalize_module_specifiers(&mut registry, &locked, &Default::default());
        assert_eq!(
            registry.modules[&shared[0]].name.as_deref(),
            Some("registerAsset")
        );
    ...
    fn uniquifies_colliding_stems_from_source_path() {
        ...
        assert_eq!(
            ...
            vec![
                "flux/Dispatcher".to_string(),
                "app_common/flux/Dispatcher".to_string()
            ]
        );
        ...
        let paths = HashMap::from([
            (
                650u32,
                "../app_common/js/packages/flux/Dispatcher.tsx".to_string(),
            ),
        ...
        assert_eq!(
            registry.modules[&650].name.as_deref(),
            Some("flux/Dispatcher")
        );
    }
}
